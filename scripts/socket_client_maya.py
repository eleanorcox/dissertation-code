...
if test_put:
    ...
    joint_pos_lists = np.ndarray(num_frames, dtype=object)
    xz_vel_lists    = np.ndarray(num_frames, dtype=object)
    for i in range(num_frames):
        joint_pos_lists[i] = joint_pos_full[i].tolist()
        xz_vel_lists[i]    = xz_vel_full[i].tolist()
